...
function viewWinAccount () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && mainWindow !== undefined && mainWindow !== null){
		...
		mainWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindConfig () {
	if(BrowserWindow.getAllWindows().length > 0 && configWindow !== undefined && configWindow !== null){
		...
		configWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindConsulta () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && consultaWindow !== undefined && consultaWindow !== null){
		...
		consultaWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindAgenda () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && agendaWindow !== undefined && agendaWindow !== null){
		...
		agendaWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindExpediente () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && expWindow !== undefined && expWindow !== null){
		...
		expWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindAdmin () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && adminWindow !== undefined && adminWindow !== null){
		...
		//adminWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
function viewWindLog () {
	...
	if(BrowserWindow.getAllWindows().length > 0 && logWindow !== undefined && logWindow !== null){
		...
		//logWindow.setIcon(path.join(__dirname, '../static/icons/icon.png'));
...
app.whenReady().then(() => {
	...
	log.info("Clinix esta listo para iniciar");
	...
	appIcon = new Tray(path.join(__dirname, '../static/icons/icon.png'));
	...
	.then((allowed) => log.info('Camera is allowed'))
	.catch((_err) => {
		log.error("camera: ", _err)
		viewWindConfig()
	});
	systemPreferences.askForMediaAccess('microphone')
	.then((allowed) => log.info('Mic is allowed'))
	.catch((_err) => {
		log.error("microphone: ", _err)
		viewWindConfig()
	});
	systemPreferences.askForMediaAccess('screen')
	.then((allowed) => log.info('ScreenShare is allowed'))
	.catch((_err) =>{
		log.error("screen: ", _err)
